...
u16 Get_LightValue(void)
{
	...
	printf("光照强度:%d\r\n",adc_data[0]);
	sprintf((char*)adc_buff,"光照强度:%d",adc_data[0]); // 拼接光照检测值
	LCD_ShowStringAndHz(95,100,150,16,16,adc_buff,0); // 起点坐标(10,20)  区域大小(150,16) 字体大小16
	return adc_data[0];
}
...
u16 Get_SmokeValue(void)
{
	...
	printf("烟雾浓度:%d\r\n",adc_data[1]);
	sprintf((char*)adc_buff,"烟雾浓度:%d",adc_data[1]); // 拼接烟雾检测值
	LCD_ShowStringAndHz(95,80,150,16,16,adc_buff,0); // 起点坐标(10,20)  区域大小(150,16) 字体大小16
	return adc_data[1];
}
