...
//	LCD_ShowPicture(0,80,16,16,(u8*)gImage_smoke); // 烟雾图标
  ...
  // PlayMP3_42(66,003); // 添加音频 (文件夹名,文件名)
	...
	sprintf(buff1,"cmd=1&uid=%s&topic=Tem,Hum,Smoke,Light,LED\r\n",PRODUCTID);
...
//				case 3: LED3_TOGGLE();PlayMP3_Smoke();break; // LED3闪烁 & 播报当前烟雾浓度
		...
		{ // 获取烟雾浓度、光照强度的值
			...
			{
				LED4(0);
				sprintf(buff1,"cmd=2&uid=%s&topic=LED&msg=#OFF#\r\n",PRODUCTID); // 拼接LED主题 -->OFF
			}
			else
			{
				LED1(1);
				sprintf(buff1,"cmd=2&uid=%s&topic=LED&msg=#ON#\r\n",PRODUCTID); // 拼接LED主题 -->ON

			}
			USART3_SendArray((u8*)buff1,strlen(buff1)); // 上传LED
