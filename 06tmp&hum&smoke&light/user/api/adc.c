...
u16 Get_LightValue(void)
{
	...
	printf("光照:%d\r\n",adc_data[0]);
	sprintf((char*)adc_buff,"光照:%d",adc_data[0]);
	LCD_ShowStringAndHz(10,60,150,16,16,adc_buff,0); // 起点坐标(10,20)  区域大小(150,16) 字体大小16
	return adc_data[0];
}
...
u16 Get_SmokeValue(void)
{
	...
	printf("烟雾:%d\r\n",adc_data[1]);
	sprintf((char*)adc_buff,"烟雾:%d",adc_data[1]);
	LCD_ShowStringAndHz(10,40,150,16,16,adc_buff,0); // 起点坐标(10,40)  区域大小(150,16) 字体大小16
	return adc_data[1];
}
